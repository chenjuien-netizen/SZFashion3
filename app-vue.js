(function() {
  ...
  const InventoryScreen = {
    ...
    components: { InventoryCard: InventoryCard, PageHeader: PageHeader },
    setup() {
          ...
          <section class="border-b border-outline-variant/20 bg-surface-container-low px-3 py-1.5 shadow-ledger">
            <div class="flex items-center gap-2">
              ...
              <datalist id="inventoryReferenceSuggestions">
                <option v-for="entry in referenceSuggestions" :key="entry.reference" :value="entry.reference">{{ entry.reference }}</option>
              </datalist>
            </div>
